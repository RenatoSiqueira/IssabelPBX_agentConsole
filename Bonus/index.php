...
function formatoAgente($agent)
{
    ...
    try {
        $conexao = new PDO('mysql:host=127.0.0.1;dbname=call_center','asterisk','asterisk',
    ...
    $sql = "SELECT name FROM agent WHERE number = :id";
    $agente = explode("/", $agent['agentchannel']);

    $Consulta = $conexao->prepare($sql);
    $Consulta->bindValue(':id',$agente[1]);
    $Consulta->execute();
